...
An invertible, differentiable map $T_\phi$ from a simple probability distribution $p(u)$ to an arbitrary one, $q (\theta)$.

It is often constructed by composing several simple transformations:

$$ T_\phi = T_1 \circ \dots \circ T_N\,.
$$

The parameters $\phi$ are computed by optimization of some loss function. 
...
::: {.incremental}
- We can get the evidence as $Z(d) \approx \langle w_i \rangle$;
- the weighted samples $(\theta_i, w_i)$ are a better representation of the posterior.
:::
